...
export default class PdfPageNumbersPlugin extends Plugin {
	...
	private isPrintRoot(el: HTMLElement) {
		return (
			...
			el.classList.contains("print-preview")
		);
	}
	...
	private applyPageNumberStyle(pageNumberEl: HTMLElement, overlay: boolean) {
		...
		if (overlay) {
			...
			style.marginBottom = isTop
				? "12px"
				: `${this.settings.marginBottom}px`;
		}
	}
	...
	private ensureMarginSpace(margins: any, requiredEdge: "top" | "bottom") {
	...
	private injectExportDialogToggle(root: ParentNode) {
		...
		if (
			...
			root.matches(".modal, .modal-container")
		) {
			modals.add(root);
		}
		const modalNodes =
	...
	private isPdfExportDialog(modal: HTMLElement) {
		...
		const exportButton = Array.from(modal.querySelectorAll("button")).some(
			(button) => /export|save/i.test(button.textContent ?? "")
		);
		const markers = ["page size", "margins", "landscape", "scale", "pdf"]
